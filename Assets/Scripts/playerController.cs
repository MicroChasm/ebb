...
public class playerController : MonoBehaviour
{
  ...
  public float fastFallLimit = -1.5f;
  ...
  public ShieldController shielded;
  float minSpeed = 2;
  [HideInInspector]
  ...
  void OnTriggerStay2D(Collider2D other)
  {
    ...
    {
            if (other.gameObject.name == "HeartofAutomata")
            {
                shield = true;
...

  {
    ...
    /* Damage System */
    BloodParticles.emissionRate = (damage);
    ...
    switch (playerState)
    {
      ...
      case PlayerState.JUMPING:
        ...
        jumpTrail *= Time.deltaTime;
                ...
                else if (!Input.GetKey("w") && !Input.GetKey("w"))
                {
                    TransitionState(PlayerState.FALLING);
                }
                else if ((verticalSpeed <= 0 ) || (jumpCounter > jumpLimit))
                {
                    TransitionState(PlayerState.FALLING);
    ...
    if (GroundedState(playerState) && playerProbes.CheckGrounded())
    {
      ...
      player.transform.position = newPosition;
        ...
        if ((playerState == PlayerState.IDLE) && (horizontalSpeed < -.05f || horizontalSpeed > .05f)){
